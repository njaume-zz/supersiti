CREATE PROCEDURE [dbo].[STR_RECUPERA_IMPORTES](
	@CAJ_ID AS INT = 0
)
AS
/*
...
WITH CTE_CIERRE (IMPORTES,TIPOIMPORTE,NOMBRE)
	AS
	(
		SELECT CASE CAE.CAE_ID
				...
				END TOTAL, CAE.CAE_ID,CAE.CAE_NOMBRE
		FROM V_CAJA_MOVIMIENTOS CAM INNER JOIN V_CAJA_TIPO_MOVIMIENTO CAE
					ON CAM.CAE_ID = CAE.CAE_ID 
		WHERE (@CAJ_ID = 0 OR (COM.CAJ_ID = @CAJ_ID))
		GROUP BY CAE.CAE_ID,CAE.CAE_NOMBRE
	)
						
SELECT IMPORTES,NOMBRE
FROM CTE_CIERRE  

...
CREATE TABLE [dbo].[V_CAJA_MOVIMIENTOS](
	...
	[CAE_ID] [INT], -- CAJA TIPO MOVIMIENTO (ABIERTA, CERRADA, CIERRE Z, ETC)
	[CAM_FECHA] [DATETIME],
	[USU_ID] [INT],
	[CAJ_ID] [INT],
	[CAM_ESTADO] [INT],
	...
	CONSTRAINT [FK_CAJA_MOVIMIENTOS_CAJA] FOREIGN KEY ([CAJ_ID]) REFERENCES [V_CAJA] ([CAJ_ID])
)
go
...
IF NOT EXISTS (SELECT *
               FROM SYSCOLUMNS
				WHERE ID = OBJECT_ID('C_PRODUCTO') AND NAME='PRO_PESABLE'
               )
               ALTER TABLE C_PRODUCTO ADD PRO_PESABLE INT NULL

...
BEGIN
	BEGIN TRANSACTION
--		BEGIN TRY
			INSERT INTO V_CAJA_MOVIMIENTOS (CAM_IMPORTE,CAM_FECHA,CAE_ID,USU_ID,CAJ_ID,CAM_ESTADO,CAM_FECHAALTA)
			VALUES (@CAM_IMPORTE,@CAM_FECHA,@CAE_ID,@USU_ID,@CAJ_ID,@CAM_ESTADO,GETDATE())
			SET @CAM_ID = @@IDENTITY
			--RETURN @CAM_ID
--		END TRY
		IF (@@ERROR != 0)
...
*/
	...
	SELECT PRO.PRO_ID,PRO.PRO_CODIGO,PRO.PRO_CODIGO_BARRA,PRO.PRO_NOMBRE,PRO.PRO_MARCA,
		   LPR.LPR_ID,LPR.PRO_ID,LPR.TPR_ID,LPR.LPR_PRECIO,LPR.LPR_PRECIOXCANTIDAD,
		   TPR.TPR_NOMBRE
	FROM C_PRODUCTO PRO LEFT JOIN C_LISTA_PRECIO LPR
		ON PRO.PRO_ID = LPR.PRO_ID INNER JOIN C_TIPOS_PRECIOS TPR
		ON LPR.TPR_ID = TPR.TPR_ID
	 WHERE (@LPR_ID = 0 OR (LPR.LPR_ID=@LPR_ID)) 
		 AND (@PRO_ID = 0 OR (LPR.PRO_ID=@PRO_ID)) 
		 AND (@TPR_ID = 0 OR (LPR.TPR_ID=@TPR_ID)) 
		 AND (LPR.LPR_ESTADO = 1)
END
...
CREATE PROCEDURE [dbo].[STR_NUEVO_COMPROBANTE_VENTA](
	@COM_ID AS INT OUTPUT,
	@COM_PTOVTA AS VARCHAR(10),
	@COM_NROEMITIDO AS VARCHAR(10),
	@COM_FECHA AS DATETIME ,
	@COM_IMPORTEGRAVADO AS NUMERIC (18,2),
	@COM_IMPORTENOGRAVADO AS NUMERIC (18,2),
	@COM_IVAFACTURADO AS NUMERIC (18,2),
	@COM_TOTALFACTRADO AS NUMERIC (18,2),
	@COM_IMPRESO AS VARCHAR(1),
	@COM_USUNOMBRE AS VARCHAR(50),
	@COM_CLIRAZONSOCIAL AS VARCHAR(100),
	@COM_CLIDOMICILIO AS VARCHAR(100),
	@COM_CLITELEFONO AS VARCHAR (50),
	@COM_CLIDOMICILIOENTREGA AS VARCHAR(100),
	@COM_CLICUIT AS VARCHAR(100),
	@COM_CLIINGRESOBRUTO AS VARCHAR(100),
	@COM_RAZONSOCIALPROPIO AS VARCHAR(100),
	@COM_DOMICILIOPROPIO AS VARCHAR(100),
	@COM_TELEFONOPROPIO AS VARCHAR(100),
	@COM_CUITPROPIO AS VARCHAR(100),
	@COM_INGRESOBRUTOPROPIO AS VARCHAR(100),
	@CTC_ID AS INT ,
	@CAJ_ID AS INT ,
	@FOP_ID AS INT
)
...
BEGIN
	BEGIN TRANSACTION
--		BEGIN TRY
			INSERT INTO V_COMPROBANTE (COM_PTOVTA,COM_NROEMITIDO,COM_FECHA,COM_IMPORTEGRAVADO,COM_IMPORTENOGRAVADO,COM_IVAFACTURADO,
						 COM_TOTALFACTRADO,COM_IMPRESO,COM_USUNOMBRE,COM_CLIRAZONSOCIAL,COM_CLIDOMICILIO,COM_CLITELEFONO,
						 COM_CLIDOMICILIOENTREGA,COM_CLICUIT,COM_CLIINGRESOBRUTO,COM_RAZONSOCIALPROPIO,COM_DOMICILIOPROPIO,
						 COM_TELEFONOPROPIO,COM_CUITPROPIO,COM_INGRESOBRUTOPROPIO,CTC_ID,CAJ_ID,FOP_ID)
			VALUES (@COM_PTOVTA,@COM_NROEMITIDO,@COM_FECHA,@COM_IMPORTEGRAVADO,@COM_IMPORTENOGRAVADO,@COM_IVAFACTURADO,
					 @COM_TOTALFACTRADO,@COM_IMPRESO,@COM_USUNOMBRE,@COM_CLIRAZONSOCIAL,@COM_CLIDOMICILIO,@COM_CLITELEFONO,
					 @COM_CLIDOMICILIOENTREGA,@COM_CLICUIT,@COM_CLIINGRESOBRUTO,@COM_RAZONSOCIALPROPIO,@COM_DOMICILIOPROPIO,
					 @COM_TELEFONOPROPIO,@COM_CUITPROPIO,@COM_INGRESOBRUTOPROPIO,@CTC_ID,@CAJ_ID,@FOP_ID)
			SET @COM_ID = @@IDENTITY
...
CREATE PROCEDURE [dbo].[STR_NUEVO_V_TIPO_COMPROBANTE](
	@CTC_ID As INT,
	@CTC_CODIGO As VARCHAR(100),
	@CTC_DESCRIPCION As VARCHAR(100),
	@CTC_SIGLA As VARCHAR(100),
	@CTC_LETRA As VARCHAR(100),
	@CTC_SIGNO As VARCHAR(100),
	@CTC_ULTIMONRO as VARCHAR(20)
) AS

BEGIN
	BEGIN TRANSACTION
		INSERT INTO V_TIPO_COMPROBANTE(CTC_CODIGO,CTC_DESCRIPCION,CTC_SIGLA,CTC_LETRA,CTC_SIGNO,CTC_ULTIMONRO)
		 VALUES (@CTC_CODIGO,@CTC_DESCRIPCION,@CTC_SIGLA,@CTC_LETRA,@CTC_SIGNO,@CTC_ULTIMONRO)
		 SET @CTC_ID= @@IDENTITY
		IF (@@ERROR != 0)
			BEGIN
				SELECT @@ERROR
...
CREATE PROCEDURE [dbo].[STR_MODIFICA_V_TIPO_COMPROBANTE](
	@CTC_ID As INT,
	@CTC_ULTIMONRO As VARCHAR(100)
) AS

BEGIN
	BEGIN TRANSACTION
		BEGIN
			UPDATE V_TIPO_COMPROBANTE 
				SET CTC_ULTIMONRO = @CTC_ULTIMONRO
			WHERE CTC_ID = @CTC_ID
		END
		IF (@@ERROR != 0)
		BEGIN
...
CREATE PROCEDURE [dbo].[STR_CONSULTA_V_TIPO_COMPROBANTE](
	@CTC_ID As INT = 0,
	@CTC_CODIGO As VARCHAR(100) = '',
	@CTC_DESCRIPCION As VARCHAR(100) = '',
	@CTC_SIGLA As VARCHAR(100) = '',
...
BEGIN
	SELECT CTC_ID,
		   CTC_CODIGO,
		   CTC_DESCRIPCION + ' ' + CTC_LETRA as 'CTC_DESCRIPCION',
		   CTC_SIGLA,
		   CTC_LETRA,
		   CTC_SIGNO,
		   CTC_ULTIMONRO
	FROM V_TIPO_COMPROBANTE
	 WHERE (@CTC_ID = 0 OR (CTC_ID=@CTC_ID)) 
		 AND (@CTC_CODIGO = '' OR (CTC_CODIGO=@CTC_CODIGO)) 
		 AND (@CTC_DESCRIPCION = '' OR (CTC_DESCRIPCION=@CTC_DESCRIPCION)) 
		 AND (@CTC_SIGLA = '' OR (CTC_SIGLA=@CTC_SIGLA)) 
		 AND (@CTC_LETRA = '' OR (CTC_LETRA=@CTC_LETRA)) 
		 AND (@CTC_SIGNO = '' OR (CTC_SIGNO=@CTC_SIGNO)) 
		 AND (@CTC_ULTIMONRO = '' OR (CTC_ULTIMONRO = @CTC_ULTIMONRO))  
END
GO

CREATE PROCEDURE [dbo].[STR_NUEVO_V_COMPROBANTE](
	@COM_ID As INT OUTPUT,
@COM_PTOVTA As VARCHAR(100),
@COM_NROEMITIDO As VARCHAR(100),
@COM_FECHA As DATETIME,
@COM_IMPORTEGRAVADO As NUMERIC(18,2),
@COM_IMPORTENOGRAVADO As NUMERIC(18,2),
@COM_IVAFACTURADO As NUMERIC(18,2),
@COM_TOTALFACTRADO As NUMERIC(18,2),
@COM_IMPRESO As VARCHAR(100),
@COM_USUNOMBRE As VARCHAR(100),
@COM_CLIRAZONSOCIAL As VARCHAR(100),
@COM_CLIDOMICILIO As VARCHAR(100),
@COM_CLITELEFONO As VARCHAR(100),
@COM_CLIDOMICILIOENTREGA As VARCHAR(100),
@COM_CLICUIT As VARCHAR(100),
@COM_CLIINGRESOBRUTO As VARCHAR(100),
@COM_RAZONSOCIALPROPIO As VARCHAR(100),
...
BEGIN
	BEGIN TRANSACTION
		INSERT INTO V_COMPROBANTE(COM_PTOVTA,COM_NROEMITIDO,COM_FECHA,COM_IMPORTEGRAVADO,COM_IMPORTENOGRAVADO,COM_IVAFACTURADO,COM_TOTALFACTRADO,COM_IMPRESO,COM_USUNOMBRE,COM_CLIRAZONSOCIAL,COM_CLIDOMICILIO,COM_CLITELEFONO,COM_CLIDOMICILIOENTREGA,COM_CLICUIT,COM_CLIINGRESOBRUTO,COM_RAZONSOCIALPROPIO,COM_DOMICILIOPROPIO,COM_TELEFONOPROPIO,COM_CUITPROPIO,COM_INGRESOBRUTOPROPIO,CTC_ID,CAJ_ID,FOP_ID)
		 VALUES (@COM_PTOVTA,@COM_NROEMITIDO,@COM_FECHA,@COM_IMPORTEGRAVADO,@COM_IMPORTENOGRAVADO,@COM_IVAFACTURADO,@COM_TOTALFACTRADO,@COM_IMPRESO,@COM_USUNOMBRE,@COM_CLIRAZONSOCIAL,@COM_CLIDOMICILIO,@COM_CLITELEFONO,@COM_CLIDOMICILIOENTREGA,@COM_CLICUIT,@COM_CLIINGRESOBRUTO,@COM_RAZONSOCIALPROPIO,@COM_DOMICILIOPROPIO,@COM_TELEFONOPROPIO,@COM_CUITPROPIO,@COM_INGRESOBRUTOPROPIO,@CTC_ID,@CAJ_ID,@FOP_ID)
		 SET @COM_ID= @@IDENTITY
		IF (@@ERROR != 0)
		BEGIN
...
CREATE PROCEDURE [dbo].[STR_MODIFICA_V_COMPROBANTE](
	@COM_ID As INT OUTPUT,
@COM_PTOVTA As VARCHAR(100),
@COM_NROEMITIDO As VARCHAR(100),
@COM_FECHA As DATETIME,
...
BEGIN
	BEGIN TRANSACTION
		UPDATE V_COMPROBANTE 
		SET COM_PTOVTA = @COM_PTOVTA,
			COM_NROEMITIDO = @COM_NROEMITIDO,
			COM_FECHA = @COM_FECHA,
			COM_IMPORTEGRAVADO = @COM_IMPORTEGRAVADO,
			COM_IMPORTENOGRAVADO = @COM_IMPORTENOGRAVADO,
			COM_IVAFACTURADO = @COM_IVAFACTURADO,
			COM_TOTALFACTRADO = @COM_TOTALFACTRADO,
			COM_IMPRESO = @COM_IMPRESO,
			COM_USUNOMBRE = @COM_USUNOMBRE,
			COM_CLIRAZONSOCIAL = @COM_CLIRAZONSOCIAL,
			COM_CLIDOMICILIO = @COM_CLIDOMICILIO,
			COM_CLITELEFONO = @COM_CLITELEFONO,
			COM_CLIDOMICILIOENTREGA = @COM_CLIDOMICILIOENTREGA,
			COM_CLICUIT = @COM_CLICUIT,
			COM_CLIINGRESOBRUTO = @COM_CLIINGRESOBRUTO,
			COM_RAZONSOCIALPROPIO = @COM_RAZONSOCIALPROPIO,
			COM_DOMICILIOPROPIO = @COM_DOMICILIOPROPIO,
			COM_TELEFONOPROPIO = @COM_TELEFONOPROPIO,
			COM_CUITPROPIO = @COM_CUITPROPIO,
			COM_INGRESOBRUTOPROPIO = @COM_INGRESOBRUTOPROPIO,
			CTC_ID = @CTC_ID,
			CAJ_ID = @CAJ_ID,
			FOP_ID = @FOP_ID
		WHERE COM_ID = @COM_ID
...
CREATE PROCEDURE [dbo].[STR_NUEVO_V_COMPROBANTE_DETALLE](
	@COD_ID As INT OUTPUT,
@COD_PROCODIGO As VARCHAR(100),
@COD_PRONOMBRE As VARCHAR(100),
@COD_PROPCIOUNITARIO As NUMERIC(18,2),
@COD_PROCANTIDAD As INT,
@COD_PRECIOCANTIDAD As NUMERIC(18,2),
@COD_IVA As NUMERIC(18,2),
@COM_ID As INT
...
BEGIN
	BEGIN TRANSACTION
		INSERT INTO V_COMPROBANTE_DETALLE(COD_PROCODIGO,COD_PRONOMBRE,COD_PROPCIOUNITARIO,COD_PROCANTIDAD,COD_PRECIOCANTIDAD,COD_IVA,COM_ID)
		 VALUES (@COD_PROCODIGO,@COD_PRONOMBRE,@COD_PROPCIOUNITARIO,@COD_PROCANTIDAD,@COD_PRECIOCANTIDAD,@COD_IVA,@COM_ID)
		 SET @COD_ID= @@IDENTITY
		IF (@@ERROR != 0)
		BEGIN
			SET @COD_ID = @@IDENTITY
...
CREATE PROCEDURE [dbo].[STR_MODIFICA_V_COMPROBANTE_DETALLE](
	@COD_ID As INT OUTPUT,
@COD_PROCODIGO As VARCHAR(100),
@COD_PRONOMBRE As VARCHAR(100),
@COD_PROPCIOUNITARIO As NUMERIC(18,2),
...
BEGIN
	BEGIN TRANSACTION
		UPDATE V_COMPROBANTE_DETALLE 
		SET COD_PROCODIGO = @COD_PROCODIGO,
			COD_PRONOMBRE = @COD_PRONOMBRE,
			COD_PROPCIOUNITARIO = @COD_PROPCIOUNITARIO,
			COD_PROCANTIDAD = @COD_PROCANTIDAD,
			COD_PRECIOCANTIDAD = @COD_PRECIOCANTIDAD,
			COD_IVA = @COD_IVA,
			COM_ID = @COM_ID
		WHERE COD_ID = @COD_ID
		IF (@@ERROR != 0)
...
CREATE PROCEDURE [dbo].[STR_CONSULTA_UltimoComprobante](
	@COM_NroComprobnte as varchar(20) OUTPUT,
	@CTC_CODIGO AS INT
) AS
BEGIN
	SELECT CTC_ULTIMONRO  AS 'COM_NroComprobnte'
	FROM v_TIPO_COMPROBANTE
...
CREATE PROCEDURE [dbo].[STR_CONSULTA_TipoComprobante](
	@CTC_CODIGO INT
)
AS
BEGIN
...
CREATE PROCEDURE [dbo].[STR_CONSULTA_ROL](
	@ROL_ID AS INT = 0,
	@ROL_NOMBRE AS VARCHAR(100)=''
)
AS
...
BEGIN
	...
	WHERE (@ROL_ID = 0 OR (ROL_ID = @ROL_ID))
		AND (@ROL_NOMBRE = '' OR (ROL_NOMBRE = @ROL_NOMBRE))
		AND ROL_ESTADO = 1
...
CREATE PROCEDURE [dbo].[STR_CONSULTA_MOVIMIENTO_CAJA](
	@CAM_ID AS INT = 0,
	@USU_ID AS INT = 0,
	@CAJ_ID AS INT = 0,
	@CAE_ID AS INT = 0
...
AS
	BEGIN
		...
		SELECT CAM.CAM_ID,CAM.CAM_IMPORTE,USU.USU_NOMBRE,CAJ.CAJ_ID,CAJ.CAJ_NUMERO,CAE.CAE_ID,CAE.CAE_NOMBRE
		FROM V_CAJA_MOVIMIENTOS CAM INNER JOIN V_CAJA CAJ 
			ON CAM.CAJ_ID = CAJ.CAJ_ID INNER JOIN V_CAJA_TIPO_MOVIMIENTO CAE
			ON CAM.CAE_ID = CAE.CAE_ID INNER JOIN S_USUARIO USU
			ON CAM.USU_ID = USU.USU_ID
		WHERE	(@CAM_ID = 0 OR (CAM.CAM_ID = @CAM_ID))
			AND (@USU_ID = '' OR (CAM.USU_ID = @USU_ID))
			AND (@CAJ_ID = 0 OR (CAJ.CAJ_ID = @CAJ_ID))
			AND (@CAE_ID = 0 OR (CAE.CAE_ID = @CAE_ID))
...
CREATE VIEW [dbo].[VT_LISTAPRECIOS] AS 
	
	SELECT PRD.PRO_ID,
			PRD.PRO_CODIGO_BARRA,
			PRD.PRO_NOMBRE,
			PRD.PRO_MARCA,
			PRD.PRO_PESABLE,
			LPR.LPR_PRECIO,
			LPR.LPR_PRECIOXCANTIDAD,
			PRD.UNC_ID,
			PRD.UNV_ID,
			FAM.FAM_NOMBRE,
			RUB.RUB_NOMBRE,
			...
			TPR.TPR_NOMBRE,
			TPR.TPR_DESCRIPCION
	FROM C_PRODUCTO PRD LEFT JOIN C_FAMILIA FAM
		ON PRD.FAM_ID = FAM.FAM_ID INNER JOIN C_RUBRO RUB
		ON FAM.RUB_ID = RUB.RUB_ID LEFT JOIN C_LISTA_PRECIO LPR

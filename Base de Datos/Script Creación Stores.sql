...
BEGIN
	BEGIN TRANSACTION
--		BEGIN TRY
			INSERT INTO V_CAJA (CAJ_NUMERO,CAE_ID,CAJ_FECHAAPERTURA,CAJ_FECHACIERRE,CAJ_ESTADO)
			VALUES (@CAJ_NUMERO,@CAE_ID,@CAJ_FECHAAPERTURA,@CAJ_FECHACIERRE,@CAJ_ESTADO)
			SET @CAJ_ID = @@IDENTITY
			--RETURN @CAJ_ID
--		END TRY
		IF (@@ERROR != 0)
...
CREATE PROCEDURE [dbo].[STR_VERIFICA_CAJA_ACCIONES](
...
SELECT * */ /*@APERTURA = (SELECT ISNULL((SELECT SUM(CAA_IMPORTE) 
								  FROM V_CAJA_ACCIONES 
								  WHERE CAE_ID = 1),0)),
	   @RETIRO= (SELECT ISNULL((SELECT SUM(CAA_IMPORTE) 
								  FROM V_CAJA_ACCIONES 
								  WHERE CAE_ID = 2),0)),
	   @CIERREX= (SELECT ISNULL((SELECT SUM(CAA_IMPORTE) 
								  FROM V_CAJA_ACCIONES 
								  WHERE CAE_ID = 3),0)),
	   @CIERREZ= (SELECT ISNULL((SELECT SUM(CAA_IMPORTE) 
								  FROM V_CAJA_ACCIONES 
								  WHERE CAE_ID = 4),0))
	*/
--SELECT @APERTURA, @RETIRO , @CIERREX, @CIERREZ 
/*
FROM V_CAJA CAJ INNER JOIN V_CAJA_ESTADO CAE
	ON CAJ.CAE_ID = CAE.CAE_ID INNER JOIN V_CAJA_ACCIONES CAA
	ON CAE.CAE_ID = CAA.CAE_ID
...
SELECT	CAJ.CAJ_ID,
		...
		CAE.CAE_NOMBRE,
		CAA.CAA_ID,
		SUM(CAA.CAA_IMPORTE) AS 'CAA_IMPORTE',
		CAA.CAA_FECHA,
		CAA.USU_ID
...
END*/
/*
SELECT (ISNULL((SELECT SUM(CAA_IMPORTE) FROM V_CAJA_ACCIONES WHERE CAE_ID = 1),0))
